...
new #[Layout('components.layouts.admin')] class extends Component
{
    ...
    {
        // Mengambil pertanyaan yang hanya dimiliki oleh kategori ini, diurutkan berdasarkan ID
        $this->pertanyaans = $this->kategori->pertanyaans()->orderBy('id')->get();
    }
    
    /**
     * Delete a question.
     */
    public function deletePertanyaan(Pertanyaan $pertanyaan): void
    {
        $pertanyaan->delete();
        ...
        <div class="bg-white p-6 rounded-lg shadow-md">
            ...
            <div class="flex justify-between items-center mb-6">
                <div class="flex items-center">
                    {{-- Diperbarui: Menggunakan nama rute dengan awalan 'admin.' --}}
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($pertanyaans as $index => $pertanyaan)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-normal text-sm text-gray-500">{{ $pertanyaan->teks_pertanyaan }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <div class="flex justify-center items-center space-x-2">
                                        <a href="{{ route('admin.kuesioner.pertanyaan.edit', ['kategori' => $kategori->id, 'pertanyaan' => $pertanyaan->id]) }}" wire:navigate class="p-2 rounded-md bg-yellow-500 text-white hover:bg-yellow-600">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                        </a>
                                        <button 
                                            wire:click="deletePertanyaan({{ $pertanyaan->id }})"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-sm text-gray-500">
                                    Belum ada pertanyaan yang dibuat untuk kategori ini.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
